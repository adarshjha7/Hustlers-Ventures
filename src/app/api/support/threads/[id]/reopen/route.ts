...
// POST — investor reopens a resolved request into the same thread ("Open this ticket again").
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (thread.status !== "CLOSED") {
    return NextResponse.json({ error: "Only a resolved request can be reopened." }, { status: 400 });
  }
